...
class DatabasePipeline:
    # Database setup
    conn = sqlite3.connect('westpac.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute(""" CREATE TABLE IF NOT EXISTS articles 
        (title text, date text, author text, tags text, link text, content text) """)
    ...
    def process_item(self, item, spider):
        ...
        # Insert values
        self.c.execute("INSERT INTO articles (title, date, author, tags, link, content)"
                       " VALUES (?,?,?,?,?,?)", (item.get('title'), item.get('date'), item.get('author'),
                                                 item.get('tags'), item.get('link'), item.get('content')))
